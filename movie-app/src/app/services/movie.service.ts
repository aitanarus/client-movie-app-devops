...
export class MovieService {
  ...
  public getPopularMovies(): Observable<MovieDetails[]> {
    const url =
  ...
  public getUpcomingMovies(): Observable<MovieDetails[]> {
    const url =
  ...
  public searchMovies(query: string): Observable<MovieDetails[]> {
    // const url =
  ...
  public getMoviePictureById(movieId: string): Observable<MovieDetails[]> {
    const options = {
      method: 'GET',
      headers: {
        accept: 'application/json',
    ...
    return new Observable((observer) => {
      fetch('https://api.themoviedb.org/3/movie/726209/images', options)
        .then((response) => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
